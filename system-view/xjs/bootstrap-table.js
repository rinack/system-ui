(function (global, factory) {
	...
	(function ($) {
		...
		var DEFAULTS = {
			...
			iconSize: undefined,
			iconsPrefix: bootstrap.iconsPrefix,
		...
		var BootstrapTable = function () {
			...
			_createClass(BootstrapTable, [{
						...
						value: function fitHeader() {
							...
							*
							* 二次变更代码
							*  去掉 .css('width', this.$el.outerWidth());
							*  与 table 内容保持一致 css 样式
							*
							* 三次变更后
							*  去掉 .css('margin-right', scrollWidth)
							*
							* 变更后代码
							* 
							*/
	   
							this.$tableHeader.find('table').html('').attr('class', this.$el.attr('class')).append(this.$header_);
